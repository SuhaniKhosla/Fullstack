...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadStoredItems() {
        ...
        storedItems.forEach(item => {
            ...
            users[item.name] = item.user.split(' ');
            prices[item.name] = item.price;
            const listItem = createListItem(item.name, users[item.name], item.price);
            listItems.appendChild(listItem);
        });
    }

    function saveToLocalStorage() {
        const items = i.map(n => ({ name: n, user: users[n].join(' '), price: prices[n] }));
        localStorage.setItem("groceryItems", JSON.stringify(items));
    }
    ...
    function addItem() {
        ...
        if (newItemText && userName) {
            const listItem = createListItem(newItemText, [userName], 0);
            listItems.appendChild(listItem);

            // Update constants without reassignment
            i.push(newItemText);
            users[newItemText] = [userName];
            prices[newItemText] = 0;
    ...
    function addPrice(button) {
        ...
        if (!isNaN(newPrice) && newPrice > 0) {
            ...
            // Update constant without reassignment
            prices[itemId] = newPrice;
    ...
    function createListItem(name, userList, price) {
        const listItem = document.createElement("li");
        listItem.innerHTML = `
            <div class="container-2" data-testid="container-2">
                <div><b>${name}</b></div>
                <div class="container-3" data-testid="${name}">
                    <div>${userList.map(user => `<div>${user}</div>`).join('')}</div>
                    <button onclick="markItem(this)">Mark</button>
                </div>
            </div>
            <div id="add-price" data-item-id="${name}">
